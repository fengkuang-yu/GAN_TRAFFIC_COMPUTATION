...
class AE:
    ...
    def build_model(self):
        ...
        inter_x = Lambda(self.random_weight_average, output_shape=self.img_shape,
                         name='random_weight_average')([real_x, imputed_img])
        
        inter_valid, inter_like = self.critic(inter_x)
        real_valid, real_like = self.critic(real_x)
        rec_valid, rec_like = self.critic(imputed_img)
        
        xent_loss = K.mean(K.binary_crossentropy(real_x, fake_x))
        like_loss = K.mean(K.square(real_like - rec_like))
        discriminate_loss = Lambda(self.critic_loss, output_shape=(1,),
                                   name='discriminate_loss')([inter_x, inter_valid, rec_valid, real_valid, ])
        
        # define generator
        self.encoder.trainable = True
        self.decoder.trainable = True
        self.critic.trainable = False
        self.auto_encoder = Model([real_x, masks], [imputed_img, real_like, rec_like])
        self.auto_encoder.add_loss(xent_loss + 100*like_loss)
        self.auto_encoder.compile(optimizer=self.optimizer)
        # self.ae.summary()
        ...
        self.discriminator = Model([real_x, masks], discriminate_loss)
        self.discriminator.add_loss(discriminate_loss)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            ae_loss = self.auto_encoder.train_on_batch([real_imgs, masks], None)
            dis_loss = self.discriminator.train_on_batch([real_imgs, masks], None)
            
            if epoch % 200 == 0:
                print('ae_loss:{};dis_loss{}'.format(ae_loss, dis_loss))
    ...
    def critic_loss(self, args):
        inter_x, inter_valid, rec_valid, real_valid, = args
        valid = -np.ones((self.batch_size, 1))
        fake = np.ones((self.batch_size, 1))
        
        rec_valid_loss = self.wasserstein_loss(fake, rec_valid)
        real_valid_loss = self.wasserstein_loss(valid, real_valid)
        penalty_loss = self.gradient_penalty_loss(None, inter_valid, inter_x)
        
        return rec_valid_loss + real_valid_loss + self.lambda_d_penalty * penalty_loss
    ...
    def plot_test(self, file_name, ):
        ...
        gen_imgs = self.auto_encoder.predict([x_test, masks], batch_size=self.batch_size)[0]
