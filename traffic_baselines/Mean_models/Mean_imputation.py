...
#-*- coding:utf-8 -*-
# author:lenovo
# datetime:2019/12/1 16:22
# software: PyCharm

# !/usr/bin/env python
# -*- coding:utf-8 -*-
# author:lenovo
# datetime:2019/12/1 10:07
# software: PyCharm
...
logging.basicConfig(format='%(asctime)s %(message)s',
                    ...
                    filename='mean_model_result.log',
                    level=logging.ERROR)
...
class mean_imputation():
    def __init__(self, missing_percentage=0.8, test_percent=0.15,
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            self.data, self.index, test_size=self.test_percent, shuffle=False)
        self.miss_mode = miss_mode
        
        # 构建模型
        self.attention_model = self.mean_imputation_model()
    
    def mean_imputation_model(self):
        x_test = self.X_test.reshape(-1, *self.img_shape)
        masks = self.mask_randomly(x_test.shape, mode=self.miss_mode)
        masked = x_test.copy()
        masked[masks == 0.] = np.nan
        
        masked_reshape = masked.reshape(-1, np.prod(x_test.shape[1:]))
        x_train = self.X_train.reshape(-1, np.prod(self.X_train.shape[1:]))
        
        x_template = x_train[:14112, :].reshape(-1, 288, 3600)
        x_template = np.mean(x_template, axis=0)
        
        x_total = np.concatenate((x_train, masked_reshape), axis=0)
        for row in range(len(x_train), len(x_total)):
            for col in range(len(x_train[0])):
                if np.isnan(x_total[row][col]):
                    x_total[row][col] = x_template[row % 288][col]
            
        x_test_imputed = x_total[-len(masked_reshape):, :]
...
if __name__ == '__main__':
    ...
    def basic_task(miss_percent=0.1):
        ...
        for miss_mode in ['patch', 'spatial_line', 'temporal_line']:
            ae = mean_imputation(missing_percentage=miss_percent, test_percent=0.15, miss_mode=miss_mode)
    
    
    # 多进程调用
    p = Pool(2)
    for i in range(9):
        basic_task(miss_percent=0.1 * (i + 1))
    logging.error('\n' + '*' * 20 + 'end' + '*' * 20)
